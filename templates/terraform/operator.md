...
If the missing variable is `md_metadata`, you are running `terraform` by hand in `src/` rather than
...
Terraform process keeps running against real infrastructure and holds the lock until it finishes on
...
Do not reach for `terraform force-unlock`. The state lives with the deployment, not in your
checkout, and the conflict carries no lock ID to force against.

## Deploy fails with `state snapshot was created by Terraform v1.13.0, which is newer than current v1.9.0`

`massdriver.yaml` says `provisioner: terraform` with no version, so the version can change between
two deploys of the same bundle. Terraform writes a version stamp into state and refuses to read a
state file written by a newer version.

Pin the version so this cannot happen again:

```yaml
steps:
  - path: src
    provisioner: terraform:1.13
```
...
deploy created it and then failed before recording it. Terraform will not adopt an object it did
not create.

The state lives with the deployment, so a local `terraform import` has nothing to write to. Adopt
it with an `import` block instead. Add it to `src/main.tf`:
...
The message shown on the canvas is a summary. The full Terraform output is in the deployment log:
