...
- One `src` step run by the OpenTofu provisioner, matching every bundle already published in this
  catalog
...
- Massdriver provider wired up in `src/providers.tf`, with commented AWS, Azure and Google blocks
  to uncomment
- Publishes no resource yet; `src/resources.tf` carries the `massdriver_resource` pattern to
  follow, and `massdriver.yaml` keeps a commented `artifacts:` block alongside it
