...
If `DATABASE_SCHEMA` is missing, link `tourdates-dev-tables` to this app on the canvas and
redeploy:

```bash
mass instance deploy tourdates-dev-dates -m "wire database connection" -f
```
...
Then, in another terminal (the password is on the `tourdates-dev-tables` instance's resource panel
in Massdriver — `psql -W` will prompt for it):

```bash
psql -h 127.0.0.1 -p 5433 -U tour_dates_app -d shared -W -c "\dn+ tour_dates"
```

## The page loads but shows "Could not reach the database: relation "tour_dates.shows" does not exist"
...
redeploy `tourdates-dev-tables`, then reload the page to let the app create the table again.

## The table is empty and the example shows never appeared
...
## A CSV upload fails with "invalid input syntax for type date"

`show_date` is a real date column, not free text. One malformed date fails the whole upload — the
rows go in as a single transaction, so nothing at all is added and the page shows the error
instead of a row count. There is no partial import to clean up.

Dates must be written `YYYY-MM-DD`:

```
city,venue,show_date
Paris,La Cigale,2026-09-21
```

`21/09/2026`, `Sept 21 2026`, and an Excel serial number like `46287` all fail. Reformat the
column in the spreadsheet before exporting, then upload the whole file again.

## A CSV upload reports "Added 0 row(s)"

Either the file had no usable lines, or every line was treated as a header.

The parser skips the first row only when its first cell reads `city`. Blank lines are dropped. A
file whose rows are all empty strings adds nothing. Columns must be in the order
`city,venue,show_date`, and extra columns past the third are discarded.
...
## Every show appears twice after an upload

Uploads append. There is no unique constraint and no duplicate check, so uploading the same file
twice inserts every row twice. Remove the extras directly (connect through the Cloud SQL Auth
Proxy as above):

```bash
psql -h 127.0.0.1 -p 5433 -U tour_dates_app -d shared -W -c "DELETE FROM tour_dates.shows a USING tour_dates.shows b WHERE a.id > b.id AND a.city = b.city AND a.venue = b.venue AND a.show_date = b.show_date;"
```
...
redeploy that component — nothing changes on this app.
...
```bash
mass deployment list tourdates-dev-dates --limit 5
```
...
```bash
mass instance deploy tourdates-dev-dates --patch='.public_access = true' -m "open to the internet" -f
```
...
```bash
mass instance orphan tourdates-dev-dates
```
...
```bash
mass bundle publish --development --bundle-directory bundles/tour-dates
```

```bash
mass instance version tourdates-dev-dates@latest+dev
```

```bash
mass instance deploy tourdates-dev-dates -m "pick up main.py change" -f
```
...
```bash
mass deployment list tourdates-dev-dates --limit 10 --status completed --action provision
```
...
```bash
mass instance deploy tourdates-dev-dates --patch='.min_instances = 1' -m "keep one copy warm" -f
```
