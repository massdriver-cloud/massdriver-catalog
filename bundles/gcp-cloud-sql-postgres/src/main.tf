...
resource "google_sql_database_instance" "main" {
  ...
  settings {
    ...
    # No public IP, ever — this instance is reachable only from the platform
    # network's private address space via the network connection's Private
    # Service Access peering. ssl_mode requires every connection (private or
    # not) to be encrypted in transit.
    ip_configuration {
      ipv4_enabled    = false
