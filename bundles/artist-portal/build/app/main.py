...
<h1>Artist Portal</h1>
<p>Running on Cloud Run. This image was built inside Google Cloud, not on anyone's laptop.</p>
<dl>
  <dt>Database</dt><dd>{database}</dd>
  <dt>My schema</dt><dd>{schema}</dd>
  <dt>Signed in as</dt><dd>{user}</dd>
</dl>
<p class="muted">This app can read and write anything inside its own schema. Every other
app's data in this database is out of reach unless somebody granted it, one table at a time.</p>
"""
