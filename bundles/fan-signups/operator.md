...
If `DATABASE_SCHEMA` is missing, link `fans-dev-tables` to this app on the canvas and redeploy:

```bash
mass instance deploy fans-dev-signups -m "wire database connection" -f
```
...
## The page loads but shows "Could not reach the database: permission denied for schema fan_signups"
...
```bash
mass instance deploy fans-dev-tables -m "reapply schema grants" -f
```
...
Then, in another terminal (the password is on the `fans-dev-tables` instance's resource panel in
Massdriver — `psql -W` will prompt for it):

```bash
psql -h 127.0.0.1 -p 5433 -U fan_signups_app -d shared -W -c "\dn+ fan_signups"
```

This table holds personal data. Prefer reading it through the app's page rather than opening a
psql session, and close the proxy when you are done.

## The page loads but shows "Could not reach the database: relation "fan_signups.signups" does not exist"
...
redeploy `fans-dev-tables`, then reload the page to let the app create the table again.

## The table is empty and the example signups never appeared
...
Worth knowing before an incident turns into a data-loss report: if real signups were deleted, the
next page load refills the table with the six fictional example people
(`j.moreau@example.com` and the rest). Seeing those names back is not recovery — it means the real
rows are gone and the table was empty when somebody opened the page. Recover from the database
backup, not from the page.
...
The parser skips the first row only when its first cell reads `email`. Blank lines are dropped. A
file whose rows are all empty strings adds nothing. Columns must be in the order
`email,name,source`, and extra columns past the third are discarded:
...
A row with fewer than three columns is padded with empty strings rather than rejected, so a file
with only email addresses imports with blank names instead of failing.
...
Uploads append, there is no unique constraint on `email`, and nothing validates the address. The
same person signing up at three shows is three rows, and re-uploading a file duplicates everything
in it.

That is deliberate — the table is a log of signups, not a deduplicated mailing list. Deduplicate
when exporting rather than deleting rows here, so the record of where somebody signed up survives.

To remove rows created by an accidental double upload (connect through the Cloud SQL Auth Proxy as
above):

```bash
psql -h 127.0.0.1 -p 5433 -U fan_signups_app -d shared -W -c "DELETE FROM fan_signups.signups a USING fan_signups.signups b WHERE a.id > b.id AND a.email = b.email AND a.source = b.source;"
```

That keeps the earliest row for each email-and-source pair, so somebody who genuinely signed up at
two different shows keeps both.

## Somebody asked to be removed from the list

Delete the rows for that address (connect through the Cloud SQL Auth Proxy as above):

```bash
psql -h 127.0.0.1 -p 5433 -U fan_signups_app -d shared -W -c "DELETE FROM fan_signups.signups WHERE email = 'j.moreau@example.com';"
```

The row is also inside whatever database backups already exist. Those age out on the shared
database's retention schedule; the deletion above does not reach back into them.

## The signup list is reachable from the internet

Check whether public access was turned on:

```bash
gcloud run services describe {{resources.service.name}} --region={{resources.service.region}} --project={{dependencies.gcp_service_account.project_id}} --format="value(spec.template.metadata.annotations,status.url)"
```

If ingress is `all` rather than `internal`, anyone with the URL can read every signup and upload
rows, because the page has no login of its own. Close it immediately:

```bash
mass instance deploy fans-dev-signups --patch='.public_access = false' -m "close public access" -f
```

The deployment history shows who turned it on and when.

## Cloud Run logs are empty — no request lines at all

Expected. The app suppresses the default per-request access log, so a healthy service writes almost
nothing to stdout. An empty log stream is not evidence that requests are failing or that the
container is not running.

This also means there is no record of who viewed the page or uploaded a file. If the question is
"who read the signup list", the Cloud Run logs cannot answer it — there is no application-level
audit trail here.
...
The build step ran but did not finish (or failed) before the wait expired. The error message itself
carries the real Cloud Build status and a direct log link — read that first:
...
```bash
mass deployment list fans-dev-signups --limit 5
```
...
`public_access` is off, which is the default and the right setting for this app. Ingress is
restricted to traffic originating inside Google Cloud, so a browser on a laptop gets a 403 no
matter who is signed in.

That 403 is the bundle working correctly. Because the page has no login of its own, turning
`public_access` on publishes every signup to anyone with the URL — see the entry above before
changing it.
...
```bash
mass instance orphan fans-dev-signups
```
...
```bash
mass bundle publish --development --bundle-directory bundles/fan-signups
```

```bash
mass instance version fans-dev-signups@latest+dev
```

```bash
mass instance deploy fans-dev-signups -m "pick up main.py change" -f
```
...
```bash
mass deployment list fans-dev-signups --limit 10 --status completed --action provision
```
...
Rolling back only changes the code and configuration. It does not restore rows — the signups
table is untouched by a rollback.
...
```bash
mass instance deploy fans-dev-signups --patch='.min_instances = 1' -m "keep one copy warm" -f
```
